...
export default function ProjectDetail() {
  ...
  const fetchPods = async () => {
    const res = await fetch(`${API_BASE}/projects/pods?projectId=${encodeURIComponent(id)}`, {
      credentials: 'include',
    });

    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.error || 'Failed to load pods');
    }
  ...
  const fetchAll = async () => {
    try {
      ...
      const metricsRes = await fetch(`${API_BASE}/projects/${id}/metrics`, { credentials: 'include' });
      ...
      const eventsRes = await fetch(`${API_BASE}/projects/${id}/events`, { credentials: 'include' });
      const eventsData = await eventsRes.json();
      if (eventsData.events && eventsData.events.length > 0) {
        setScalingEvents(eventsData.events);
      }

      const historyRes = await fetch(`${API_BASE}/projects/${id}/scaling-history`, {
        credentials: 'include',
      });
  ...
  const handleStartAutoScale = async () => {
    try {
      const res = await fetch(`${API_BASE}/projects/${id}/start-autoscale`, {
        method: 'POST',
        credentials: 'include',
  ...
  const handleStopAutoScale = async () => {
    try {
      const res = await fetch(`${API_BASE}/projects/${id}/stop-autoscale`, {
        method: 'POST',
        credentials: 'include',
      });
      ...
      if (res.ok) {
        ...
        alert('Failed to stop auto-scaling: ' + (error.error || 'Unknown error'));
      }
    } catch (error) {
      alert('Error: ' + error.message);
    }
  ...
  const displayEvents =
    scalingEvents.length > 0
      ...
      : [
        {
          event: `${runningPods} pod${runningPods !== 1 ? 's' : ''} running`,
          time: 'Just now',
          color: '#16a34a',
        },
        {
          event: 'Deployment created',
          time: project?.created_at ? new Date(project.created_at).toLocaleTimeString() : '—',
          color: '#3b82f6',
        },
      ];
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      >
        ...
        >
            <div>
              <div style={{ fontWeight: '700', fontSize: '16px', color: '#111827' }}>Auto-Scaling Control</div>
              <div style={{ fontSize: '13px', color: '#6b7280', marginTop: '4px' }}>
                Traffic-driven auto-scaling with ML predictions when available
              </div>
            </div>
          <div
            style={{
        ...
        {currentTraffic && (
          ...
          >
            ...
            <div style={{ fontSize: '12px', color: '#166534' }}>
              Traffic: <strong>{currentTraffic.trafficData.requests_per_sec} req/s</strong> → Replicas:{' '}
              <strong>
                {currentTraffic.scaling.previousReplicas} → {currentTraffic.scaling.requiredReplicas}
              </strong>
          ...
          <div style={{ marginTop: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '200px', overflowY: 'auto' }}>
              {scalingHistory.slice(0, 10).map((event, i) => (
                ...
                >
                  <div>
                    <strong>{event.requests_per_sec} req/s</strong> → Scaled from {event.old_replicas} to{' '}
                    {event.new_replicas} pods
                  </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
        ...
        <MetricCard
          ...
          value={loading ? '—' : clusterStatus}
          sub="K8s default namespace"
          color={clusterColor}
        />
        <MetricCard
          icon="⚡"
        ...
        />
        <MetricCard icon="💾" label="Memory" value={totalMemMB > 0 ? `${totalMemMB}MB` : '—'} sub="Across all pods" />
      </div>
